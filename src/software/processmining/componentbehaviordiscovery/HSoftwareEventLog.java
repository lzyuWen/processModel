...
public class HSoftwareEventLog {
	...
	public XLog getTopLevelLog() {
        if (subLogMapping != null && !subLogMapping.isEmpty()) {
            // Iterate over the sub logs and find the top-level log
            for (HSoftwareEventLog subLog : subLogMapping.values()) {
                if (subLog.getMainLog() != null) {
                    return subLog.getTopLevelLog();
                }
            }
        }
        
        // If no sub logs with main log found, return this log as top-level
        return mainLog;
    }
