...
public class HEventLogConstruction {
			...
			XLogInfo Xloginfo, String Component)
	{
		...
		// Get the event classifier
		XEventClassifier eventClassifier = Xloginfo.getEventClassifier();
		
		// Create XEventClasses object using the event classifier and original log
		XLogInfo logInfo = XLogInfoFactory.createLogInfo(mainLog, eventClassifier);
		//XEventClasses eventClasses = Xloginfo.getEventClasses(originalLog, eventClassifier);
		XEventClasses eventClasses = logInfo.getEventClasses(eventClassifier);
		...
		if (xeventclass2Eventlist.size()>0 && maximalNestingDepth>1)// there exists nested events for the top-level log
		{
			...
			for(XEventClass xeventc : xeventclass2Eventlist.keySet())
			{
				subLogMapping.put(xeventc, 
						...
						originalLog, Xloginfo, eventClasses, Component));
			}
		}
	...
	public static HSoftwareEventLog ConstructHierarchicalLogRecusively(int maximalNestingDepth, int currentNestingDepth, 
			...
			HashSet<XEvent> eventSet, XLog originalLog, XLogInfo Xloginfo, XEventClasses eventClasses, String Component)
	{
