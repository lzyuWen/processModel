...
public class creteHPN33 {
	
		@Plugin(name = "AAAAAcreate HPN33", level = PluginLevel.Regular, returnLabels = { "LOG" }, returnTypes = {
		...
		public XLog mineCAM(UIPluginContext context,XLog xlog) {
         ...
         for(int i = 0;i<98;i++) {
             ...
             XEvent e1 = ModifyLog.makeEventSimpleness("d", "start");
          
             XEvent e2 = ModifyLog.makeEventSimpleness("d", "complete");

             xTrace.add(e1);
             xTrace.add(e2);
             xLog.add(xTrace);
         }
         for(int i = 98;i<194;i++) {
        	 XAttributeMap xTraceAttributeMap = new XAttributeMapImpl();
             xTraceAttributeMap.put("concept:name", new XAttributeLiteralImpl("concept:name", i+"", XConceptExtension.instance()));
             XTrace xTrace = new XTraceImpl(xTraceAttributeMap);          
